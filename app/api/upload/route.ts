...
export async function POST(req: Request) {
  try {
    ...
    if (!ALLOWED_TYPES.includes(file.type)) {
      return err("Only JPEG, PNG, WebP and GIF are allowed", 400);
    }
    ...
    // SightEngine moderation with 8s timeout
    // If it fails for any reason, we allow the upload and log it
    try {
      ...
      // SightEngine failed or timed out - log but allow upload
      console.error("[upload] moderation skipped:", modError);
    }
